...
t = {
    "English": {
        ...
        "act_title": "💾 Actions & Reports", "save": "💾 Save to History", "dl_pdf": "📄 Download as a PDF", "dl_xlsx": "📥 Detailed Excel",
    ...
    "Français": {
        ...
        "act_title": "💾 Actions & Rapports", "save": "💾 Sauvegarder dans l'Historique", "dl_pdf": "📄 Télécharger en PDF", "dl_xlsx": "📥 Excel Détaillé",
    ...
    "Español": {
        ...
        "act_title": "💾 Acciones y Reportes", "save": "💾 Guardar en Historial", "dl_pdf": "📄 Descargar como PDF", "dl_xlsx": "📥 Excel Detallado",
    ...
    "العربية": {
        ...
        "act_title": "💾 الإجراءات والتقارير", "save": "💾 حفظ في السجل", "dl_pdf": "📄 تنزيل كملف PDF", "dl_xlsx": "📥 تقرير Excel",
...
if uploaded_file:
    try:
        ...
        # CHANGED TO 3 COLUMNS TO FIT EXCEL EXPORT
        ...
        with c_action3:
            # ENHANCED EXCEL EXPORT WITH 3 SHEETS AND FORMATTING
            output_excel = io.BytesIO()
            with pd.ExcelWriter(output_excel, engine='xlsxwriter') as writer:
                # Sheet 1: Financials
                df_display.to_excel(writer, sheet_name="Financials")
                
                # Sheet 2: KPIs
                kpi_data = {
                    "Metric": [txt["nm"], txt["roe"], txt["cr"]],
                    "Value": [f"{user_net_margin:.2f}%", f"{user_roe:.2f}%", f"{current_ratio:.2f}x"]
                }
                pd.DataFrame(kpi_data).to_excel(writer, sheet_name="KPIs", index=False)
                
                # Sheet 3: Diagnosis
                pd.DataFrame({"Diagnosis Notes": selected_nbs}).to_excel(writer, sheet_name="Expert_Diagnosis", index=False)
                
                # Style Header for Financials
                ...
                for col_num, value in enumerate(["Line Item"] + list(df_display.columns)):
                    ...
                    worksheet.set_column(col_num, col_num, 20) # Widen columns
